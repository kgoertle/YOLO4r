...
def get_checkpoint_and_resume(mode, resume_flag, runs_dir: Path, default_weights=None, custom_weights=None):
    checkpoint = None
    ...
    # ---- Update mode: use best.pt ----
        ...
        if not checkpoint:
            ...
            if default_weights:
                checkpoint = ensure_weights(Path(default_weights))

    # ---- Transfer-learning custom weights ----
    elif mode == "train" and custom_weights:
        checkpoint = ensure_weights(Path(custom_weights))
